            ...
            if count2 > 1:
                for i in range(len(values2)):
                    if i == 2:
                        ...
                        if dic['BussinessApp'].replace('_', '-').split('-')[1] == "Mule":
                            dic[heading2[i]] = f"https://artifactory.tfs.toyota.com/artifactory/easc-maven-prod-local/artifacts/{'-'.join(dic['BussinessApp'].replace('_', '-').split('-')[3:]).strip().lower()}.jar"
                        else:
                            dic[heading2[i]] = f"https://artifactory.tfs.toyota.com/artifactory/easc-maven-prod-local/artifacts/{'-'.join(dic['BussinessApp'].replace('_', '-').split('-')[2:]).strip().lower()}.jar"
...
with open('JenkinsJobs.csv', 'w', newline='') as f:
